...
The clipboard carries none of this, so it has to be asked of the OS — and the
two platforms want opposite shapes, because asking costs wildly different
amounts.

On **Windows** the answer needs P/Invoke, which means PowerShell: about 430ms of
shell startup plus 570ms compiling the interop type. Doing that on every copy
would be absurd, so a small helper is started once — on your first copy, not at
launch — and then answers in about ten milliseconds. It is stopped again after
ten minutes of nothing being copied, rather than sitting there all night.

On **macOS** `lsappinfo` answers directly and cheaply, so nothing is kept alive
and the question is simply asked each time. It ships with the system and needs
no Accessibility permission, unlike asking System Events through osascript,
which would prompt.
